...
namespace 医院门诊
{
    public partial class Phymanagersdenglu : Form
    {
        public Phymanagersdenglu()
        {
            InitializeComponent();
        }
        ...
        private void btndenglu_Click(object sender, EventArgs e)
        {
            ...
            sqlCommand.CommandText =                                                        //指定SQL命令的命令文本；命令文本由字符串拼接而成；
             $"SELECT COUNT(1) FROM tb_Phymanagers" +
             $" WHERE phymanagersno=@no" +								//将文本框的文本清除首尾的空格后，拼接至命令文本中；
             $" AND Password=HASHBYTES('MD5',@password);";
            sqlCommand.Parameters.AddWithValue("@no", this.txtno.Text.Trim());
            sqlCommand.Parameters.AddWithValue("@password", this.txtmima.Text.Trim());
            ...
            else                                                                            //否则；
            {
                ...
                this.txtmima.Focus();                                                  //密码文本框获得焦点；
                this.txtmima.SelectAll();                                              //密码文本框内所有文本被选中；
            }
        }
    }
}
